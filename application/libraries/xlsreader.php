...
/* vim: set expandtab tabstop=2 shiftwidth=4 softtabstop=4: */
...
require_once APPPATH.'libraries/olereader.php';
...
//exit;
	...
	*/
		...
		$res = $this->olereader->read($sFileName);

		// oops, something goes wrong (Darko Miljanovic)
		if($res === false) {
			// check error code
			if($this->olereader->error == 1) {
			// bad file
		...
		$this->data = $this->olereader->getWorkBook();
...
//        var_dump($res);
		...
		$total = $this->olereader->ppsTotal();
		for ($i = 0; $i < $total; $i++) {
			if ($this->olereader->isFile($i)) {
				$type = unpack("v", $this->olereader->getData($i, 0, 2));
				if ($type[''] == 0x0809)  { // check if it's a BIFF stream
					$this->_index = $i;
					$this->data = $this->olereader->getData($i, 0, $this->olereader->getDataLength($i));
					break;
				}
			}
		}
...
/* End of file xlsreader.php */
/* Location: ./application/libraries/xlsreader.php */
