...
select c.course_id, subject_id, code, year, c.name as subjectname, shortname,ep.title as papername, 
starttime, endtime,
c.visible, c.status,
s.paper_id, getEnrollCount(c.course_id) AS enrollcount,count(stu_id) AS testedcount,
...
left join exam_papers ep on s.paper_id = ep.paper_id
